...
Webbgränssnittet finns sedan på `http://localhost:5000` (eller den port du satt `WEBUI_PORT` till i `.env` - `docker-compose.yml` läser samma fil både för att skicka in miljövariabler i containern och för att avgöra vilken värdport som mappas), och är precis som utanför Docker bara nåbart från den egna maskinen som standard. `WEBUI_HOST` i `.env` har ingen effekt här - `docker-compose.yml` sätter alltid `WEBUI_HOST=0.0.0.0` inne i containern (nödvändigt för att Dockers egen portpublicering ska nå fram överhuvudtaget) och styr istället den faktiska exponeringen via sin `ports:`-rad, som i sin tur bara binder till värdmaskinens `127.0.0.1` som standard. Ska webbgränssnittet nås från en annan dator (samma server/VM-fall som utan Docker), ändra den raden i `docker-compose.yml` direkt - se kommentaren där. Ett enskilt kommando istället för webbgränssnittet körs som `docker compose run --rm kvittomall fetch` (byt `fetch` mot vilket kommando som helst, t.ex. `status` eller `run`).
